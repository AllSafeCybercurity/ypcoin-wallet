...
#if defined(NDEBUG)
# error "Ypcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x17895bcaaf84f80415cfd0cf484a5d22a4bfa30c5633bfb91cf4f9bd9661a577");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ypcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ypcoin Signed Message:\n";
...
{
    // Ypcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Ypcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Ypcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Ypcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ypcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ypcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x92e8008b014e835d5088499c52120cd0c11cae563e9718435b9f99b26fecae3c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "In A.I God We Thrust && Verify";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049d72366135130e20bcc779fbcfac7c76b60a60e60b086ff8ec5ad146a0b7f7d744454a910ef2d3ada0f60c44c6ad8694e2bcc3a530c35f619027b5802c8d7b67") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1567963395;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084703605;

        if (fTestNet)
        {
            block.nTime    = 1567963314;
            block.nNonce   = 385676177;
        }  
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x0256949015bec37fb9d7e88af8bcbe9ed3160d63b94d662b6cea980420c9a368"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0xc1, 0xb7, 0xdd }; // Ypcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YpcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YpcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YpcoinMiner(CWallet *pwallet)
{
    printf("YpcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ypcoin-miner");
    ...
    try { loop {
        ...
        printf("Running YpcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YpcoinMiner, pwallet));
}
